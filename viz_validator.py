...
Author: Jon Wright / latent-affect
License: MIT
"""
...
def png_pixel_diff_pct(bytes_a: bytes, bytes_b: bytes) -> float:
    ...
    try:
        # Fallback: decode PNG with stdlib zlib (works for non-interlaced PNGs)
        import zlib, struct
        ...
        # Last resort: byte-level difference as rough proxy
...
AUTO_SELECTORS = [
    ...
    ".tabs button",
    ".tab-bar button",
    ".controls button:not(#rotate-btn)",   # MAIF Visualizer specific
]

AUTO_INJECT_PATTERNS = [
    # function name patterns to look for in page source
    "loadJSON",
    "loadData",
    "loadFile",
    "ingestData",
...
# ---------------------------------------------------------------------------
#  MAIF Visualizer — specific JSON injection
# ---------------------------------------------------------------------------

def inject_maif_json(page, json_path: Path, pause: float = 0.8) -> bool:
    """
    ...
    try:
        ...
        if inputs:
            ...
            page.wait_for_timeout(int(pause * 2000))  # wait for FileReader + render
            return True
...
def run_validation(
    ...
    with sync_playwright() as p:
        ...
        if data_path:
            ...
            # Try MAIF-style direct file injection first
            try:
                injected = inject_maif_json(page, data_path, pause)
            except Exception as e:
        ...
        # We scroll to it once so canvases are in view, then screenshot that element.
        container_el = page.evaluate(f"""() => {{
            const btn = document.querySelector({json.dumps(selector)});
            if (!btn) return null;
            // Walk up to find the parent section, then get the canvas container
            let el = btn;
        ...
        # This avoids false-positives when clicking the already-active tab.
        labels = list(all_shots.keys())
        ...
        for i, btn in enumerate(buttons):
            ...
            # Find max diff against any other tab
